...
export const createStyleContext = <R extends Recipe>(recipe: R) => {
        ...
        const Comp = forwardRef<any, Override<ComponentProps<C>, NonNullable<VariantProps<R>>>>((props, ref) => {
            ...
            return (
                <StyleContext.Provider value={slotRecipe}>
                    <Component ref={ref} {...(props as any)} className={cx(slotRecipe[slot ?? ''], localProps.className)} />
    ...
    const withContext = <C extends ElementType>(Component: C, slot?: Slot<R>): C => {
        const Comp = forwardRef((props: ComponentProps<C>, ref) => {
            ...
            return <Component ref={ref} {...(props as any)} className={cx(recipe?.[slot ?? ''], props.className)} />;
        });
