 ...
 * @returns
 */
export function useForageSortedArray<T>(groupKey: string, isValid: (x: unknown) => boolean, defaultArray: T[] = [], keyBy?: (x: T) => string) {
    ...
    async function reorder(newOrder: string[]) {
        if (!newOrder.every((x) => typeof x === 'string')) throw new Error('invalid order');
        setOrder(newOrder);
