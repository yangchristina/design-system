...
export default function useForageIdArray<K, T>(
    ...
    async function init() {
        ...
        const items = await Promise.all(keys.map(async (key) => localforage.getItem(((keyBy ? keyBy(key) : key) as number | string).toString())))
    ...
    async function setValue(id: any, value: T | T & Function) {
        ...
        if (newVal)
            await localforage.setItem(id.toString(), newVal)
    }
